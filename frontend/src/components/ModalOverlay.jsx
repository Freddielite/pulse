...
// Matches App.css's .pl-overlay--closing/.pl-modal--closing animation
...
// .pl-overlay--closing class - see App.css), and only then actually
...
export default function ModalOverlay({ open, onCancel, closeOnBackdrop = true, children }) {
  ...
  return createPortal(
    <div className={`pl-overlay${closing ? " pl-overlay--closing" : ""}`} onClick={closeOnBackdrop ? onCancel : undefined}>
      {children}
    </div>,
    document.body
  );
