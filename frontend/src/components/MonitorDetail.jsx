...
export default function MonitorDetail({ monitor, currentUser, existingGroups = [], onBack, onChanged, toast }) {
  ...
  useEffect(() => {
    ...
    let ignore = false;
    getOrganization(monitor.organization_id)
      .then((org) => {
        if (!ignore) setOrgBrand(org);
      })
      .catch(() => {});
    return () => {
      ignore = true;
  ...
  // BASE is the frontend's own API base (an absolute URL in production,
  // or a relative "/api" in local dev behind a proxy) - absolutized here
  // since this URL is meant to be pasted onto someone else's site, where
  // a relative path would resolve against the wrong host entirely.
  const badgeUrl = monitor.share_token
  ...
  async function handleCopyBadgeEmbed() {
    try {
      await navigator.clipboard.writeText(badgeEmbedHtml);
  ...
  function handleDownloadReport() {
    ...
    const reportBrand = orgBrand?.brand_name?.trim() || "Pulse";
    const lines = [
      `SECURITY REPORT`,
          ...
          <>
            ...
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: "1px solid var(--panel-border)" }}>
              ...
              <div style={{ fontSize: 11.5, color: "var(--ink-faint)", marginBottom: 8 }}>
                Drop this on the monitored site, in a README, or an email signature - shows the current grade and
                30-day uptime, updates on its own, no login required to view it.
              </div>
              <img src={badgeUrl} alt={`${monitor.name} security badge`} width={168} height={42} style={{ display: "block", marginBottom: 8 }} />
